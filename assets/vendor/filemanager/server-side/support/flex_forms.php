...
?>
	...
	protected function ProcessField($num, &$field, $id)
	{
		...
		?>
					<div class="fieldtablewrap<?php if ($this->state["firstitem"])  echo " firstitem"; ?>">
						<table class="rowwrap">
							<tbody>
								<tr>
								<?php
								...
								<div class="formitemdata">
									...
									?>
													<div class="<?= $mode ?>itemwrap" <?php echo $stylewidth; ?>>
														<input class="<?= $mode ?>" type="<?= $mode ?>" id="<?php echo $id2; ?>" name="<?php echo htmlspecialchars($field["name"]); ?><?php if ($mode == "checkbox")  echo "[]"; ?>" value="<?php echo htmlspecialchars($name2); ?>" <?php if (isset($field["select"][$name2]))  echo " checked"; ?><?php if ($this->state["autofocused"] === $id) {
																																																																																						echo " autofocus";
																																																																																						$this->state["autofocused"] = true;
																																																																																					} ?> />
												...
												?>
												<div class="<?= $mode ?>itemwrap" <?php echo $stylewidth; ?>>
													<input class="<?= $mode ?>" type="<?= $mode ?>" id="<?php echo $id2; ?>" name="<?php echo htmlspecialchars($field["name"]); ?><?php if ($mode == "checkbox")  echo "[]"; ?>" value="<?php echo htmlspecialchars($name); ?>" <?php if (isset($field["select"][$name]))  echo " checked"; ?><?php if ($this->state["autofocused"] === $id) {
																																																																																					echo " autofocus";
																																																																																					$this->state["autofocused"] = true;
																																																																																				} ?> />
											...
											<tbody>
												...
												?>
														<tr<?php foreach ($trattrs as $key => $val)  echo " " . $key . "=\"" . htmlspecialchars($val) . "\""; ?>>
															...
															foreach ($row as $col) {
															?>
																<td<?php if (isset($colattrs2[$num2])) {
																		foreach ($colattrs2[$num2] as $key => $val)  echo " " . $key . "=\"" . htmlspecialchars($val) . "\"";
																	} ?>><?php echo $col; ?></td>
																<?php
																$num2++;
															}
										...
										<div class="nontablewrap" id="<?php echo htmlspecialchars($idbase); ?>" <?php if (isset($field["width"]))  echo " style=\"" . ($this->state["responsive"] ? "max-" : "") . "width: " . htmlspecialchars($field["width"]) . "\""; ?>>
											...
											?>
													<div<?php foreach ($trattrs as $key => $val)  echo " " . $key . "=\"" . htmlspecialchars($val) . "\""; ?>>
														...
														foreach ($row as $col) {
														?>
															<div<?php if (isset($headcolattrs[$num2])) {
																	foreach ($headcolattrs[$num2] as $key => $val)  echo " " . $key . "=\"" . htmlspecialchars($val) . "\"";
																} ?>><?php echo (isset($field["htmlcols"]) && $field["htmlcols"] ? self::FFTranslate(isset($field["cols"][$num2]) ? $field["cols"][$num2] : "") : htmlspecialchars(self::FFTranslate(isset($field["cols"][$num2]) ? $field["cols"][$num2] : ""))); ?>
										</div>
										<div<?php if (isset($colattrs2[$num2])) {
																foreach ($colattrs2[$num2] as $key => $val)  echo " " . $key . "=\"" . htmlspecialchars($val) . "\"";
															} ?>><?php echo $col; ?>
								</div>
							<?php
...
?>
	...
	var obj = window;
	path = path.split('.');
	for (var x = 0; x < path.length; x++) { if (obj[path[x]]===undefined) return; obj=obj[path[x]]; } return obj; }; $this.ProcessJSQueue=function() { ready=true; for (var name in jsqueue) { if (jsqueue.hasOwnProperty(name)) { if ((jsqueue[name].loading===undefined || jsqueue[name].loading===false) && (jsqueue[name].dependency===false || jsqueue[jsqueue[name].dependency]===undefined)) { if (jsqueue[name].detect !==undefined && $this.GetObjectFromPath(jsqueue[name].detect) !==undefined) delete jsqueue[name]; else if (jsqueue[name].mode==="src" ) LoadJSQueueItem(name); else if (jsqueue[name].mode==="inline" ) { jsqueue[name].src(); delete jsqueue[name]; } } } } CheckEmptyAndNotify(); }; $this.Init=function() { if (ready) $this.ProcessJSQueue(); else if (!initialized) { if (document.addEventListener) { function regevent(event) { document.removeEventListener("DOMContentLoaded", regevent); $this.ProcessJSQueue(); } document.addEventListener("DOMContentLoaded", regevent); } else { setTimeout($this.ProcessJSQueue(), 250); } initialized=true; } }; }; window.FlexForms=new FlexFormsInternal(); })(); FlexForms.SetVersion('<?php echo self::JSSafe($this->version); ?>'); <?php
