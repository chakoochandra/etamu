<div class="d-flex">
    <?php echo filter_form($form, 'site/history') ?>
</div>
        ...
        <?php if ($data) : ?>
            <?php foreach ($data as $i => $row) : ?>
                <tr <?php echo $row->status == 1 ? 'class="bg-danger"' : ($row->status == 2 ? 'class="bg-success"' : 'class="bg-default"') ?>>
                    ...
                    <td><?php echo $row->address ?><?php echo $row->phone_number ? "<br/><span style='white-space: nowrap;'><i class='fa fa-whatsapp'></i>&nbsp;$row->phone_number</span>" : '' ?><?php echo $row->email ? "<br/><span style='white-space: nowrap;'><i class='fa fa-at'></i>&nbsp;$row->email</span>" : '' ?></td>
                    <td align="center">
                    ...
                    <td align="center" style="white-space: nowrap;">
                        <?php echo $row->photo ? anchor(base_url("site/photo/{$row->id}"), '<i class="fa fa-user-circle-o"></i>', [
                            'title'    => 'Lihat Foto',
                            'class'    => 'btn btn-xs btn-outline-primary btn-modal',
                        ]) : '' ?>
                        <?php echo anchor(base_url("site/update/{$row->id}"), '<i class="fa fa-pencil"></i>', [
                            'title'    => 'Perbarui',
                            'class'    => 'btn btn-xs btn-outline-warning btn-modal',
                        ]) ?>
                        <?php echo anchor_confirm(base_url("site/delete/{$row->id}"), '<i class="fa fa-trash"></i>', [
                            'title'    => 'Hapus',
